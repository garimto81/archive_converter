...
> **v3.2.0 업데이트 (2025-12-12)**: NAS 전체 스캔 분석 결과, 5개 → 24개 패턴으로 확장. 매칭률 15.8% → 98.4% 개선.

#### 8.2.1 매칭률 현황

| 버전 | 패턴 수 | 매칭 파일 | 매칭률 |
|------|---------|----------|--------|
| v3.1 (이전) | 5개 | 217/1,374 | 15.8% |
| v3.2 (현재) | 24개 | 1,352/1,374 | **98.4%** |

#### 8.2.2 패턴 카테고리별 분류

| 카테고리 | 패턴 수 | 매칭 파일 | 대표 패턴 |
|----------|---------|----------|----------|
| WSOP Archive | 7 | 588 | `year_world_series`, `wsop_year_space` |
| WSOP Modern | 4 | 378 | `hand_clip`, `wsop_main_event` |
| WSOP Short Code | 5 | 220 | `ws_short_code`, `wsop_yy_code` |
| WSOP Europe | 4 | 35 | `wsope_yy_episode`, `we_code` |
| Circuit/Paradise | 3 | 103 | `circuit_subclip`, `paradise_subclip` |
| Other Brands | 5 | 108 | `pad_episode`, `gog_episode` |
    ...
    for pattern_name, pattern in FILENAME_PATTERNS.items():
        ...
        if match:
            ...
            return FileNameMeta(
                ...
                year_code=groups.get("year"),
                sequence_num=int(groups["num"]) if groups.get("num") else None,
                clip_type=groups.get("type"),
                raw_description=groups.get("desc") or groups.get("player") or groups.get("tags"),
                season=int(groups["season"]) if groups.get("season") else None,
                episode=int(groups["episode"]) if groups.get("episode") else None,
                event_number=int(groups["event"]) if groups.get("event") else None,
