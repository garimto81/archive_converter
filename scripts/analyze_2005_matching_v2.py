"""
2005년 NAS vs PokerGO 매칭 분석 스크립트 v2
...
def parse_event_number(filename: str) -> Optional[int]:
    ...
    # Show NN 패턴
    match = re.search(r'show\s+(\d+)', filename, re.IGNORECASE)
    if match:
        return int(match.group(1))
...
def extract_pokergo_features(video: Dict) -> Dict:
    ...
    # 이벤트 번호
    match = re.search(r'(?:event\s*|wsop\s+2005\s+)(\d+)', title, re.IGNORECASE)
    if match:
...
def extract_nas_features(file: Dict) -> Dict:
    ...
    features = {
        ...
        'main_event_day': None,
        'is_toc': 'tournament of champ' in filename.lower() or 'toc' in filename.lower(),
        'is_commentary': 'commentary' in filename.lower(),
        'is_main_event': 'eoe' in filename.lower() or 'main' in filename.lower(),
    }
...
def match_videos(pokergo_features: Dict, nas_features_list: List[Dict]) -> Tuple[Optional[Dict], str]:
    ...
    for nas_features in nas_features_list:
        ...
        if (pokergo_features['main_event_day'] is not None and
            ...
            score += 8
            reasons.append(f"ME Day {pokergo_features['main_event_day']}")
...
def main():
    ...
    # 2. NAS 파일 목록 (상위 20개)
    lines.append("\n" + "="*80)
    lines.append("2. NAS 파일 목록 (상위 20개)")
    lines.append("="*80)
    for i, features in enumerate(nas_features_list[:20], 1):
        ...
        extra = " | ".join(filter(None, [event_info, day_info, ", ".join(flags)]))
        lines.append(f"{i:3d}. {features['file_name']}")
        if extra:
            lines.append(f"     → {extra}")
    ...
    lines.append("="*80)
    lines.append(f"{'PokerGO Title':<60} | {'NAS File':<50} | {'매칭 근거':<30}")
    lines.append("-" * 145)
    ...
    for pokergo_features in pokergo_features_list:
        ...
        if nas_match:
            ...
            lines.append(f"{title:<60} | {filename:<50} | {reason:<30}")
        else:
            unmatched_pokergo.append(pokergo_features['title'])
            lines.append(f"{pokergo_features['title']:<60} | {'매칭 없음':<50} | {'':<30}")

    # 4. 추가 NAS 파일 개수
    unmatched_nas_count = len(nas_files) - len(matched_nas_files)

    lines.append("\n" + "="*80)
    lines.append("4. 매칭되지 않은 NAS 파일")
    lines.append("="*80)
    lines.append(f"총 {unmatched_nas_count}개 파일\n")
    ...
    if unmatched_nas_count > 10:
        lines.append(f"     ... 외 {unmatched_nas_count - 10}개")

    # 5. Summary 통계
    lines.append("\n" + "="*80)
    lines.append("5. Summary 통계")
    lines.append("="*80)
    lines.append(f"PokerGO 총 영상:      {len(pokergo_videos):4d}개")
    lines.append(f"NAS 총 파일:          {len(nas_files):4d}개")
    lines.append("")
    lines.append(f"매칭 성공:            {matched_count:4d}개")
    lines.append(f"매칭 실패 (PokerGO):  {len(unmatched_pokergo):4d}개")
    lines.append(f"매칭 실패 (NAS):      {unmatched_nas_count:4d}개")
    lines.append("")
    lines.append(f"매칭률 (PokerGO):     {matched_count/len(pokergo_videos)*100:5.1f}%")
    lines.append(f"매칭률 (NAS):         {len(matched_nas_files)/len(nas_files)*100:5.1f}%")
    ...
    for match_type, count in sorted(match_types.items(), key=lambda x: -x[1]):
        lines.append(f"  - {match_type:<20}: {count:3d}개")
    ...
    print(f"\n요약:")
    print(f"  - PokerGO: {len(pokergo_videos)}개")
    print(f"  - NAS: {len(nas_files)}개")
    print(f"  - 매칭 성공: {matched_count}개 ({matched_count/len(pokergo_videos)*100:.1f}%)")
    print(f"  - 매칭 실패 (PokerGO): {len(unmatched_pokergo)}개")
    print(f"  - 매칭 실패 (NAS): {unmatched_nas_count}개")
